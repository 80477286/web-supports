Ext.define('Extend.form.field.TreeComboBox', {
	...
	alias : ['widget.treecombobox', 'widget.TreeComboBox'],
	uses : ['Ext.tree.Panel'],
	config : {
		/**
		...
		me.picker = Ext.create('Extend.tree.Panel', {
					...
					listeners : {
						...
						afterrender : function() {
							Ext.defer(function() {
										if (!me.picker.getStore().autoLoad
												&& !me.picker.autoLoad) {
						...
						checkchange : function(node, checked, eOpts) {
							var me = this.picker;
							me.fireEvent('selectionchange', this.picker, me
											.getChecked())
						},
		...
		if (node.isRoot()) {
			if (node.isLoaded() && node.isLeaf() !== true) {
				...
				node.expand(false, function() {
							for (var i = 0; i < node.childNodes.length; i++) {
								if (Ext.String.startsWith(value, node
												.get(me.valueField))) {
									var item = node.childNodes[i];
									me.selectByValue(item, value);
								}
