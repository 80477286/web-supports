Ext.define('Extend.form.Basic', {
	...
	findField : function(id) {
		return this.getFields().findBy(function(field) {
			return field.id === id || field.name === id
					|| Ext.String.startsWith(field.name, id + '.')
					|| field.dataIndex === id;
		});
	},
				...
				if (Ext.isObject(data)) {
					for (name in data) {
						if (data.hasOwnProperty(name)) {
							...
							fullName = Ext.isEmpty(me.entity)
									? name
									: (me.entity + ".") + name;
							if (!field.isRadio) {
	...
	getDatas : function(asString, dirtyOnly, includeEmptyText, useDataValues,
		...
		for (var key in values) {
			...
			if (key.indexOf('.') > -1) {
				var property = key.substring(key.lastIndexOf('.') + 1,
						key.length);
	...
	createObjects : function(target, path) {
		if (path.indexOf('.') != -1) {
			...
			if (isArray) {
				...
				var index = Math.abs(head.replace(property, "").replace(
						/[\[]|[\]]/g, ''));
				if (!target[property]) {
					target[property] = [];
				}
