Ext.define('Extend.form.field.CheckCombo', {
	...
	alias : ['widget.checkcombo', 'widget.CheckCombo',
			'widget.checkboxcombobox', 'widget.checkcombobox'],
	multiSelect : true,
	lastQuery : '',
	...
	initComponent : function() {
		...
		this.tpl = Ext
				.create(
						...
						'</style>', '<ul class="x-list-plain"><tpl for=".">',
						'<li role="option" class="x-boundlist-item">'
								+ (me.multiSelect === true
										? '<div class="x-combo-checker">'
										: '') + '</div><div>{'
								+ this.displayField + '}<div></li>',
						'</tpl></ul>')
				...
				if (!Ext.isEmpty(nv)) {
					if (Ext.isArray(nv)) {
						...
						Ext.Array.each(nv, function(item) {
									var list = store.query(me.valueField, item);
									for (var i = 0; i < list.getCount(); i++) {
										list.getAt(i).set('checked', true);
									}
									if (list.getCount() > 0) {
										sv.push(item);
									}
						...
						if (sv.length != nv.length) {
							Extend.Msg.error('错误', "字段：“" + me.getName()
											+ '”中部份选项在下拉列表中已经不存在，已经移除，请重新保存！',
									{
										原 : nv,
										现 : sv
									});
	...
	getChecked : function() {
		var me = this;
		var store = me.getStore();
		var list = store.query('checked', true);
		var arr = [];
		for (var i = 0; i < list.getCount(); i++) {
			arr.push(list.getAt(i));
		}
		return arr;
	},
